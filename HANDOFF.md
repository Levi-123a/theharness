...
class LLMProvider(ABC):
    ...
    # summarize_session 有默认实现（从输入推导），OpenAILLMProvider 覆盖为直接 API 调用

class MockLLMProvider(LLMProvider):
    def __init__(self, actions: list[dict[str, Any]]) -> None: ...
    def complete(self, messages) -> dict[str, Any]: ...  # 返回 {"action", "params", "reasoning"}
    def reset(self) -> None: ...
    def summarize_session(self, task_desc, action_summaries, success, reason) -> str: ...  # 确定性返回，不消耗预设 action
...
class MemoryStore:
    ...
    def delete_sessions(self, session_ids: list[int]) -> int: ...  # 批量删除，返回实际删除数
